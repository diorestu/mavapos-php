...
@section('content')
    <div x-data="{ 
        showAddModal: @js($errors->any() && ($errors->has('name') || $errors->has('code') || $errors->has('unit'))), 
        showStockModal: @js($errors->any() && $errors->has('quantity')) 
    }" class="space-y-4">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                <button type="button" @click="showStockModal = true"
                    class="inline-flex h-10 items-center justify-center gap-1.5 rounded-lg border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-white/[0.04]">
                ...
                <button type="button" @click="showAddModal = true"
                    class="inline-flex h-10 items-center justify-center gap-1.5 rounded-lg bg-brand-500 px-4 text-sm font-semibold text-white shadow-theme-xs transition hover:bg-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M12 5v14M5 12h14"/>
                    </svg>
                    Tambah Bahan Baku
                ...
                <table class="w-full min-w-[820px]">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900/40">
                            <th class="px-4 py-3 text-left"><p class="text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Bahan</p></th>
                            <th class="px-4 py-3 text-center"><p class="text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Kode</p></th>
                            <th class="px-4 py-3 text-right"><p class="text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Stok</p></th>
                    ...
                    <tbody>
                        @forelse ($materials as $material)
                            <tr class="border-b border-gray-100 transition-colors last:border-0 hover:bg-gray-25 dark:border-gray-800 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="px-4 py-2.5 text-center text-[12px] font-medium text-gray-500 dark:text-gray-400">{{ $material->code }}</td>
                                <td class="px-4 py-2.5 text-right text-[12px] font-semibold tabular-nums text-gray-800 dark:text-white/90">{{ $decimal($material->stock) }} {{ $material->unit }}</td>
                                <td class="px-4 py-2.5 text-right text-[12px] tabular-nums text-gray-500 dark:text-gray-400">{{ $decimal($material->min_stock) }} {{ $material->unit }}</td>
                                <td class="px-4 py-2.5 text-right text-[12px] font-semibold tabular-nums text-gray-800 dark:text-white/90">{{ $rupiah($material->cost_per_unit) }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    @if ((float) $material->stock <= 0)
                                        <span class="rounded-full bg-error-50 px-2.5 py-0.5 text-[11px] font-semibold text-error-600 dark:bg-error-500/15 dark:text-error-400">Habis</span>
                                    @elseif ((float) $material->min_stock > 0 && (float) $material->stock <= (float) $material->min_stock)
                                        <span class="rounded-full bg-warning-50 px-2.5 py-0.5 text-[11px] font-semibold text-warning-700 dark:bg-warning-500/15 dark:text-orange-400">Menipis</span>
                                    @else
        ...
        <div x-cloak x-show="showAddModal" class="fixed inset-0 z-99999 flex items-center justify-center p-4">
            ...
            <div x-show="showAddModal"
                ...
                @if ($errors->any() && ($errors->has('name') || $errors->has('code') || $errors->has('unit')))
                    <div class="mt-3 rounded-lg border border-error-200 bg-error-50 px-4 py-2.5 text-xs text-error-700 dark:border-error-500/20 dark:bg-error-500/10 dark:text-error-300">
                        {{ $errors->first() }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('raw-materials.store') }}" class="mt-4 space-y-3">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" value="{{ old('name') }}" type="text" placeholder="Contoh: Gula aren" required
                    ...
                    <div>
                        ...
                        <textarea id="note" name="note" rows="2" placeholder="Catatan bahan baku"
                    ...
                    <div class="mt-4 flex gap-2">
                        <button type="button" @click="showAddModal = false" class="h-10 flex-1 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.04]">
                            Batal
                        </button>
                        <button type="submit" class="h-10 flex-1 rounded-lg bg-brand-500 text-sm font-semibold text-white shadow-theme-xs hover:bg-brand-600">
                            Simpan
                        </button>
                    </div>
                </form>
            ...
            <div x-show="showStockModal"
                ...
                <form method="POST" class="mt-4 space-y-3" x-data="{ action: '' }" :action="action">
                    ...
                    <div>
                        ...
                        <input id="stock_quantity" name="quantity" type="number" min="0.001" step="0.001" placeholder="0" required
                            class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-right text-sm tabular-nums text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                    </div>
                    ...
                    <div class="mt-4 flex gap-2">
                        <button type="button" @click="showStockModal = false" class="h-10 flex-1 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.04]">
                            Batal
                        </button>
                        <button type="submit" :disabled="!action" class="h-10 flex-1 rounded-lg bg-brand-500 text-sm font-semibold text-white shadow-theme-xs hover:bg-brand-600 disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500 dark:disabled:bg-gray-700 dark:disabled:text-gray-400">
                            Tambah Stok
                        </button>
                    </div>
                </form>
