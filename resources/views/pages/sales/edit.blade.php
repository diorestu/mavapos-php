...
@section('content')
    ...
    <script>
        function saleEditor(sale, items) { const existing = Object.fromEntries((sale.payments || []).map(p => [p.payment_method, p.amount])); return { items, lines: sale.items.map(i => ({ id: i.product_variant_id ? 'variant-' + i.product_variant_id : 'product-' + i.sku, quantity: i.quantity })), payment_method: sale.payment_method, discount: sale.discount, paid_amount: sale.paid_amount, payments: [{method: 'cash', label: 'Tunai', amount: existing.cash || 0}, {method: 'qris', label: 'QRIS', amount: existing.qris || 0}, {method: 'card', label: 'Kartu', amount: existing.card || 0}], buyer_nationality: sale.buyer_nationality || '', reason: '', loading: false, error: '', async submit() { this.loading = true; this.error = ''; const response = await fetch(@js(route('sales.update', $sale)), { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content }, body: JSON.stringify({ items: this.lines, payment_method: this.payment_method, discount: this.discount || 0, paid_amount: this.paid_amount || 0, payments: this.payment_method === 'split' ? this.payments.filter(p => Number(p.amount) > 0).map(p => ({method: p.method, amount: Number(p.amount)})) : null, buyer_nationality: this.buyer_nationality || null, reason: this.reason }) }); if (response.ok) { window.location = @js(route('sales')); return; } const data = await response.json(); this.error = data.message || 'Koreksi gagal disimpan.'; this.loading = false; } } }
    </script>
@endsection
